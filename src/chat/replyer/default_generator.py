...
**核心要点**:
- **分段核心**: 在一个相对独立的想法说完后，就插入 `[SPLIT]` 进行分段。这就像真人在聊天时，说完一小句就按下发送键一样。
- **倾向短句**: 请优先将回复组织成更短的句子分段发送，这样能让对话显得更轻松、自然。
- **标点处理**: `[SPLIT]` 标记本身就代表了停顿，所以在插入它的位置，请确保删除了前面的逗号、句号等标点符号。

**任务**:
请基于以上要点，像一个真人在聊天一样，自然地决定在哪里插入 `[SPLIT]` 标记以进行分段。
"""
            # 将分段指令添加到提示词顶部
